...
ccPowerMultiPop <- function(grr, pval, maf1, maf2, flip)
{
	...
	NCP.values <- matrix(NA, nrow=1, ncol=2)
	NCP.values[1,1] <- ncp1
	NCP.values[1,2] <- ncp2

	write.table(NCP.values, file=paste("NCP.values.", pop2, ".", sign, ".", bin, ".bin",  sep=""), append=TRUE, col.names=FALSE)
...
repeat {
	...
	bin <- commandArgs()[x+3]; bin <- substr(bin, 2, nchar(bin))
	sign <- commandArgs()[x+4]; sign <- substr(sign, 2, nchar(sign))
        break
        }
...
print(grr); print(pop2); print(bin); print(sign)

inputfile <- paste("founders.freq.", sign, ".", bin, ".bin", sep="")
outputfile <- paste("power.NCP.", pop2, ".", sign, ".", bin, sep="")
