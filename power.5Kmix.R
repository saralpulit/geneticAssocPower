...
ccPowerMultiPop <- function(grr, pval, maf1, maf2, maf3, maf4, maf5, maf6, maf7, flip)
{
	...
	n_samples3 <- n_case3 + n_control3
	n_samples4 <- n_case4 + n_control4
	n_samples5 <- n_case5 + n_control5
	n_samples6 <- n_case6 + n_control6
	n_samples7 <- n_case7 + n_control7
	n_joint <- n_samples1 + n_samples2 + n_samples3 + n_samples4 + n_samples5 + n_samples6 + n_samples7
	...
	# calculate ncp for stage 2, pop1, calling cc_gpc function
	if ( raf2 > 0 && raf2 < 1 ) {
		ncp2 <- cc_gpc( raf2, k, grr, grr^2, n_case2, n_control2, F )
	}
	
	# calculate ncp for stage 2, pop2, calling cc_gpc function
	if ( raf3 > 0 && raf3 < 1 ) {
		ncp3 <- cc_gpc( raf3, k, grr, grr^2, n_case3, n_control3, F )
	}
	
	# calculate ncp for stage 2, pop3, calling cc_gpc function
	if ( raf4 > 0 && raf4 < 1 ) {
		ncp4 <- cc_gpc( raf4, k, grr, grr^2, n_case4, n_control4, F )
	}
	
	# calculate ncp for stage 2, pop4, calling cc_gpc function
	if ( raf5 > 0 && raf5 < 1 ) {
		ncp5 <- cc_gpc( raf5, k, grr, grr^2, n_case5, n_control5, F )
	}
	
	# calculate ncp for stage 2, pop5, calling cc_gpc function
	if ( raf6 > 0 && raf6 < 1 ) {
		ncp6 <- cc_gpc( raf6, k, grr, grr^2, n_case6, n_control6, F )
	}
	
	# calculate ncp for stage 2, pop6, calling cc_gpc function
	if ( raf7 > 0 && raf7 < 1 ) {
		ncp7 <- cc_gpc( raf7, k, grr, grr^2, n_case7, n_control7, F )
	}
	
	# sum the ncp values for stage 2 and use that cumulative ncp to calculate stage 2 power
	ncp.sum <- ncp2 + ncp3 + ncp4 + ncp5 + ncp6 + ncp7
		
	# call power.meta to calculate power for stage 1, stage 2, and joint power (if ncp for either stage == 0)
	power <- power.meta( ncp1, ncp.sum, pval )
...
ccPowerMultiPopBigLoop <- function( afceu, afpop2, afpop3, afpop4, afpop5, afpop6, afpop7 )
{
	# each iteration (e.g. results.pop.number) yields three numbers - power 1, power 2, joint.power
	
	results.flip.F <- ccPowerMultiPop( grr, 5e-8, afceu, afpop2, afpop3, afpop4, afpop5, afpop6, afpop7, F )
	results.flip.T <- ccPowerMultiPop( grr, 5e-8, afceu, afpop2, afpop3, afpop4, afpop5, afpop6, afpop7, T )
...
repeat { 
	...
	pop3 <- commandArgs()[x+3]; pop3 <- substr(pop3, 2, nchar(pop3))
	pop4 <- commandArgs()[x+4]; pop4 <- substr(pop4, 2, nchar(pop4))
	pop5 <- commandArgs()[x+5]; pop5 <- substr(pop5, 2, nchar(pop5))
	pop6 <- commandArgs()[x+6]; pop6 <- substr(pop6, 2, nchar(pop6))
	pop7 <- commandArgs()[x+7]; pop7 <- substr(pop7, 2, nchar(pop7))
	letter <- commandArgs()[x+8]; letter <- substr(letter, 2, nchar(letter))
	break
	}
...
inputfile <- paste("founders.freq.all.", letter, ".hm3.txt", sep="")
outputfile <- paste("power.5K.MIX.80K.MKK.", letter, ".", grr, ".txt", sep="")

# input allele frequencies
freqs <- read.table(inputfile, header=T, stringsAsFactors=F, as.is=T)
	
# run power calculations
meta <- mapply( ccPowerMultiPopBigLoop, freqs$AFCEU, freqs[,pop2], freqs[,pop3], freqs[,pop4], freqs[,pop5], freqs[,pop6], freqs[,pop7] )
