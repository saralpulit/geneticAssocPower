...
ccPowerMultiPop <- function(grr, pval, maf1, maf2, maf3, maf4, flip)
{
	...
	n_joint <- n_samples1 + n_samples2 + n_samples3 + n_samples4
...
ccPowerMultiPopBigLoop <- function( afceu, afpop2, afpop3, afpop4 )
{
	# each iteration (e.g. results.pop.number) yields three numbers - power 1, power 2, joint.power
	
	results.flip.F <- ccPowerMultiPop( grr, 5e-8, afceu, afpop2, afpop3, afpop4, F )
	results.flip.T <- ccPowerMultiPop( grr, 5e-8, afceu, afpop2, afpop3, afpop4, T )
...
repeat { 
	...
	letter <- commandArgs()[x+5]; letter <- substr(letter, 2, nchar(letter))
	break
	}
...
outputfile <- paste("power.MAJCONT.80K.", letter, ".", grr, ".txt", sep="")

# input allele frequencies
freqs <- read.table(inputfile, header=T, stringsAsFactors=F, as.is=T)
	
# run power calculations
meta <- mapply( ccPowerMultiPopBigLoop, freqs$AFCEU, freqs[,pop2], freqs[,pop3], freqs[,pop4] )
